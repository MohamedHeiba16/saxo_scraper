...
class BestsellerSpider(scrapy.Spider):
    ...
    start_urls = ["https://www.saxo.com/dk/bestsellere?pt=e-bog"]
    ...
    def start_requests(self):
        yield scrapy.Request(self.start_urls[0],
                            ...
                            PageMethod("click", "#CybotCookiebotDialogBodyLevelButtonLevelOptinAllowAll"),
                            PageMethod("wait_for_timeout", 2000),
                            PageMethod("wait_for_selector", "li[class='product-list__item product-list__item--row']"),
                            PageMethod("wait_for_timeout", 2000),
    ...
    # for title in response.xpath("//li[@class='product-list-info__title']/a/text()"): 
        #     yield {
        #         "titles":title.get()
        #     }

    def parse(self, response):
        for url in response.xpath("//li[@class='product-list-info__title']/a/@href"):
            relative_url = "https://www.saxo.com/" + url.get()
            yield response.follow(relative_url,callback=self.parse_results)

    def parse_results(self,response):
        title = response.xpath("//h1[@class='text-xl sm:text-l text-800 mb-0']/text()").get()
        Rank = response.xpath("//a[@class='tags-list__item']/text()").get()
        AuthorName = response.xpath("//div[@class='mb-16']//a[@title][1]/text()").get()
        Price_normal = response.xpath("//label[@for='price-Normal']/span[@class='text-l text-800']/text()").get()
        ...
        yield {
            ...
            "Price_normal":Price_normal,
                            ...
                            # Another way to do the project with api 
